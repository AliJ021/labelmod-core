-- =====================================================================
-- ۰۲۶ — هزینه‌ای که «به بهای کالا نمی‌رود»، واقعاً نرود
-- =====================================================================
-- `purchasing.receipt_charge.allocation` از روز اول سه مقدار داشت:
-- `by_value`، `by_qty` و **`none`**. دو تای اول کار می‌کردند. سومی
-- نه — و خرابی‌اش بی‌صدا بود.
--
-- ── باگ ──────────────────────────────────────────────────────────────
--
-- `post_receipt` باقی‌مانده گرد کردن را با **کل** هزینه می‌سنجید:
--
--     IF v_alloc_sum <> v_charges THEN  -- v_charges شامل none هم بود
--       آخرین سطر += (v_charges - v_alloc_sum)
--
-- سطرها فقط سهم `by_value` و `by_qty` را گرفته بودند، پس اختلاف
-- دقیقاً برابر هزینهٔ `none` می‌شد و **تمامش روی آخرین سطر می‌نشست**.
--
-- یعنی گزینه‌ای که می‌گفت «این هزینه به بهای کالا نرو»، در عمل همه‌اش
-- را روی یک قلم دلخواه می‌گذاشت. سند متوازن می‌ماند و هیچ تستی قرمز
-- نمی‌شد؛ فقط بهای تمام‌شده آن یک قلم بی‌دلیل بالا می‌رفت و سود همان
-- قلم کم گزارش می‌شد.
--
-- تا امروز هیچ‌کس نمی‌توانست به این مسیر برسد (رسید خرید API نداشت).
-- با ساخته‌شدن صفحه خرید، این گزینه در دسترس کاربر قرار می‌گیرد —
-- پس پیش از آن باید درست شود.
--
-- ── اصلاح ────────────────────────────────────────────────────────────
--
--   • باقی‌مانده گرد کردن با «هزینه‌ای که وارد بها می‌شود» سنجیده
--     می‌شود، نه با کل هزینه
--   • هزینه `none` یک سطر سند جدا می‌گیرد: مؤلفه تازه `expensed_charge`
--   • حساب هزینه اختیاری است و روی خودِ سطر هزینه می‌نشیند؛ نداده
--     یعنی حساب پیش‌فرض قاعده ثبت
--
-- توازن سند دست‌نخورده می‌ماند: جمع بدهکار همان است، فقط میان
-- «موجودی کالا» و «هزینه دوره» تقسیم می‌شود.
--
-- ⚠️ پس از این مهاجرت `ops/db.sh seed` را اجرا کنید — قاعده ثبت
--    `expensed_charge` از `db/seed/020_posting_rules.sql` می‌آید.
--    تا آن موقع، رسید **بدون** هزینهٔ `none` مثل قبل کار می‌کند
--    (سطر با مبلغ صفر اصلاً ساخته نمی‌شود، پس قاعده هم خوانده
--    نمی‌شود).
-- =====================================================================
...
ALTER TABLE purchasing.receipt_charge
  ADD COLUMN expense_account_code text REFERENCES ledger.account(code);

COMMENT ON COLUMN purchasing.receipt_charge.expense_account_code IS
  'حساب هزینه دوره — فقط برای allocation = none. تهی یعنی حساب پیش‌فرض قاعده ثبت.';

-- حساب هزینه روی هزینه‌ای که به بهای کالا می‌رود بی‌معناست: آن مبلغ
-- اصلاً به حساب هزینه نمی‌خورد، به موجودی کالا می‌خورد. اگر پرش کنیم،
-- کاربر فکر می‌کند انتخابش اثری دارد.
ALTER TABLE purchasing.receipt_charge
  ADD CONSTRAINT charge_expense_account_only_when_none
  CHECK (expense_account_code IS NULL OR allocation = 'none');

-- ---------------------------------------------------------------------
-- ثبت رسید — با تفکیک هزینهٔ واردشونده به بها از هزینهٔ دوره
-- ---------------------------------------------------------------------
