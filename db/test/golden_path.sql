...
-- اعداد این سناریو با **میانگین متحرک موزون** نوشته شده‌اند (تخصیص
-- هزینه حمل، میانگین دو رسید، COGS فروش). پیش‌فرض سیستم از مهاجرت
-- ۰۱۳ «آخرین قیمت خرید» است، پس روش را صریح انتخاب می‌کنیم.
--
-- روش پیش‌فرض جای دیگری سنجیده می‌شود و بی‌پوشش نمانده:
-- `db/test/costing.sql` کل چرخه خرید تا فروش را با «آخرین قیمت خرید»
-- می‌رود و برابری سود دو روش را ادعا می‌کند، و
-- `db/test/auto-close.sql` و `db/test/price-override.sql` هم روی
-- همان پیش‌فرض اجرا می‌شوند.
...
PERFORM platform.set_setting('costing.method', '"moving_weighted_average"'::jsonb,
  'سناریوی طلایی: اعدادش با میانگین موزون نوشته شده', v_user);
