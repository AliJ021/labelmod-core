...
-- نقش سایت — کلاینت ماشینی افزونه ووکامرس، نه یک آدم.
--
-- یک **نقش** است و نه یک استثنا در کد: مجوزش از همان
-- `permission_rule` می‌آید که همه می‌آیند، پس «سایت چه می‌تواند بکند»
-- یک SELECT است، نه خواندن سورس.
('web',        'سایت (ووکامرس)')
ON CONFLICT (code) DO NOTHING;
...
-- ── نقش سایت ─────────────────────────────────────────────────────────
--
-- سه ردیف، و هر سه لازم‌اند:
--
--   sale.create          سفارش سایت یک فاکتور است، مثل هر فروش دیگر.
--
--   sale.price_override  سایت **باید** قیمت بفرستد. مشتری همان چیزی را
--                        پرداخت کرده که سایت نشانش داده؛ اگر ما قیمت
--                        جاری `catalog.price` را ثبت کنیم و سایت وسط
--                        کمپین چیز دیگری گرفته باشد، دفتر درآمدی
--                        می‌نویسد که هرگز دریافت نشده.
--
--   sale.discount        سقف «کاهش کل» — همان نردبانی که صندوق از آن
--                        می‌گذرد. **بی‌سقف است و این یک تصمیم است، نه
--                        فراموشی:** پول را درگاه قبلاً گرفته و سفارشی
--                        که ثبت نشود، درآمدی است که هیچ‌جا نیست —
--                        نه در دفتر، نه در انبار، و کسی هم دنبالش
--                        نمی‌گردد. رد کردن سفارشِ پرداخت‌شده، مغایرت
--                        درست نمی‌کند؛ پنهانش می‌کند.
--
--                        ⚠️ اگر مالک بخواهد کمپین سایت سقف داشته باشد،
--                        یک UPDATE روی `max_percent` همین سطر است:
--                        آن‌وقت سفارشِ بالای سقف ۴۰۹ می‌گیرد و افزونه
--                        در صف Retry می‌ماند تا آدمی تصمیم بگیرد.
--
