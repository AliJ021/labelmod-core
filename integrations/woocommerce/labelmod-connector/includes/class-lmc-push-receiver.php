 ...
 *   Nonce      یک‌بارمصرف، در Transient
 *   نسخه       پیام قدیمی یا تکراری دور انداخته می‌شود
 *
...
class LMC_Push_Receiver
{
    ...
    const WINDOW           = 300;
    const NONCE_PREFIX     = 'lmc_push_nonce_';
    ...
    public static function verify_request($request)
    {
        ...
        // Nonce یک‌بارمصرف. عمرش دو برابر پنجره است تا تا وقتی Timestamp
        // معتبر است، Nonce هم به‌یاد باشد.
        $key = self::NONCE_PREFIX . $nonce;
        if (get_transient($key) !== false) {
            return new WP_Error('lmc_replay', 'این درخواست قبلاً پردازش شده است.', ['status' => 409]);
        }
        set_transient($key, 1, self::WINDOW * 2);

        return true;
    }

    /** بدنهٔ مشترک هر دو مسیر: پیدا کردن کالا و سنجش نسخه. */
    private static function resolve($request, string $version_meta): array
    {
        ...
        //    انداخته شود. تحویل «حداقل یک بار» است، پس همین حالت عادی
        //    است نه استثنا — و بی این، یک Retry بی‌دلیل `post_meta` را
        //    می‌نویسد و هر Hook وابسته را بیدار می‌کند.
        if ($stored > 0 && $incoming <= $stored) {
            return ['skip' => 'نسخهٔ قدیمی یا تکراری'];
        }

        return ['data' => $data, 'product_id' => $product_id, 'version' => $incoming];
    }

    public static function handle_stock($request)
    {
        $r = self::resolve($request, self::STOCK_VERSION_META);
        ...
        $changed = LMC_Stock_Sync::apply_stock($product, (int) $data['onHand']);
        if ($changed) { $product->save(); }

        // ⚠️ نسخه **حتی وقتی چیزی عوض نشده** ثبت می‌شود. وگرنه یک پیام
        //    با عدد بدون تغییر (مثلاً پس از انتقال داخلی) نسخه را جلو
        //    نمی‌برد و پیام بعدیِ قدیمی‌تر باز هم پذیرفته می‌شد.
        update_post_meta($r['product_id'], self::STOCK_VERSION_META, $r['version']);

        return ['ok' => true, 'applied' => $changed, 'onHand' => (int) $data['onHand']];
    }

    public static function handle_price($request)
    {
        $r = self::resolve($request, self::PRICE_VERSION_META);
        if (isset($r['error'])) { return $r['error']; }
        if (isset($r['skip']))  { return ['ok' => true, 'applied' => false, 'note' => $r['skip']]; }
        ...
        $changed = LMC_Stock_Sync::apply_price($product, $price);
        if ($changed) { $product->save(); }

        update_post_meta($r['product_id'], self::PRICE_VERSION_META, $r['version']);

        return ['ok' => true, 'applied' => $changed, 'price' => $price];
    }
}
