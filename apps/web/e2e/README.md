...
Playwright is pinned in package.json and pnpm-lock.yaml. CI runs Chromium and WebKit in both themes at widths 320, 375, 768, 1024, 1440 and 1920. Eight scenarios per project produce 192 browser cases. Tests use only synthetic data; unexpected API requests, external requests and uncaught browser errors fail the test. Money fields follow the API's string contract. This suite does not replace PostgreSQL integration tests.

Visual artifacts (catalog, product detail, settings and dashboard) wait for actual local font loads; screenshots and failure traces are uploaded by CI. Layout checks cover long names, wide tables and 200% text enlargement. Images require human review; they are not a claim of automatic aesthetic scoring.
