...
/**
 * نام عمل برای عملیاتی که **بدنه‌اش قابل ویرایش است**.
 *
 * سرور بدنه را با بدنه ذخیره‌شده همان کلید مقایسه می‌کند و اگر فرق
 * داشته باشد `idempotency_key_reused` می‌دهد. پس اگر نام عمل فقط از
 * شناسه ساخته شود، این اتفاق می‌افتد:
 *
 *   ۱. صندوق‌دار فرم را می‌فرستد و شبکه قطع است
 *   ۲. مبلغ را **اصلاح می‌کند** و دوباره می‌فرستد
 *   ۳. همان کلیدِ نگه‌داشته‌شده می‌رود ولی بدنه فرق دارد → ۴۰۹
 *   ۴. هر تلاش بعدی همان ۴۰۹ — **تا Reload صفحه گیر می‌کند**
 *
 * و پیام آن خطا («کلید تازه بفرستید») برای برنامه‌نویس نوشته شده،
 * نه برای کسی که پای صندوق ایستاده.
 *
 * راه‌حل همان چیزی است که مسیر پرداخت از اول داشت: مبلغ **داخل** نام
 * عمل است (`pay:<فاکتور>:<دریافتی>`)، پس عوض‌شدنش کلید تازه می‌سازد.
 * اینجا همان قاعده برای بدنه‌های بزرگ‌تر عمومی شده.
 *
 * ⚠️ همان شیء‌ای را بده که به سرور می‌رود. هدف این است که کلید دقیقاً
 * وقتی عوض شود که **سرور هم** بدنه را متفاوت می‌بیند — نه زودتر، نه
 * دیرتر. زودتر یعنی Retry ساده اثر دوم می‌سازد؛ دیرتر یعنی همان بن‌بست.
 */
export function actionFor(prefix: string, payload: unknown): string {
