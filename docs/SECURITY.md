...
- **نقش دیتابیس runtime تولید:** API و Worker پیش از listen/پردازش صف، نقش واقعی
  را بررسی می‌کنند؛ نقش دارای اختیارات مدیریتی یا مالکیت دیتابیس/اسکیما/رابطه
  (مستقیم یا از طریق عضویت قابل استفاده) رد می‌شود. Compose رمز مستقل برنامه را
  اجباری می‌کند. ابزار مهاجرت و بکاپ اتصال مالک جدا دارند؛ `createDb` عمومی و
  داربست آزمون مالک محدود نمی‌شوند.
- **دامنه دستگاه و نشست:** دستگاه تازهٔ بدون شعبه فقط با مدیر دارای دامنهٔ همهٔ شعب
  تأیید و به شعبه تخصیص داده می‌شود. مدیریت بعدی هم شعبهٔ فعلی و هم مقصد را بررسی می‌کند.
  فهرست نشست و قطع دسترسی کاربر، همهٔ نقش‌های هدف را با دامنهٔ عامل می‌سنجند؛
  دیدن نشست دستگاه خارجی مجاز نیست. تغییر نقش و شعبه با قفل تراکنشی هماهنگ می‌شود.
- **اعتبارسنجی با Zod در هر مرز API.** هیچ ورودی بدون Schema وارد منطق نشود.
- **دامنه تنظیمات پایانه:** دیدن و تغییر، علاوه بر مجوز، به شعبهٔ حساب وابسته‌اند.
  حساب مشترک برای شعبه‌ها خواندنی است؛ تغییر کارمزد، دوره تسویه یا درایور آن
  فقط برای کاربر دارای دامنهٔ همهٔ شعب مجاز است. نام بانک مقصد خارج از دامنه
  نمایش داده نمی‌شود. هنگام تغییر، حساب و دامنهٔ کاربر در همان تراکنش قفل و بررسی شوند.
